...
const ORBIT_R  = 250; // orbit radius px
const ICON_BOX = 52;  // icon card size px
...
const FEATURE_CARDS = [
  {
    icon: MessageCircle,
    title: 'Hỏi đáp AI',
    desc: 'Giải đáp thắc mắc về môn học, bài tập, kiến thức chuyên ngành',
  },
  {
    icon: BookOpen,
    title: 'Ôn tập & Luyện đề',
    desc: 'Tổng hợp tài liệu, đề thi, bài tập và luyện đề theo chủ đề',
  },
  {
    icon: GraduationCap,
    title: 'Cố vấn học tập',
    desc: 'Tư vấn chọn môn, kế hoạch học tập, định hướng ngành nghề',
  },
  {
    icon: Map,
    title: 'Lộ trình học tập',
    desc: 'Cá nhân hóa lộ trình học tập theo mục tiêu của bạn',
  },
  {
    icon: Bell,
    title: 'Nhắc lịch & Theo dõi',
    desc: 'Nhắc lịch học, lịch thi, theo dõi tiến độ và kết quả học tập',
  },
];
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans overflow-x-hidden" style={{ background: '#eef4ff' }}>
      ...
      <header className="sticky top-0 z-50 w-full px-6 py-5">
        <div
          className="w-full flex items-center gap-6 px-5 h-14 rounded-2xl"
          style={{
            ...
            boxShadow:
              '0 4px 24px rgba(99,140,255,0.10), 0 1px 0 rgba(255,255,255,0.8) inset, 0 -1px 0 rgba(147,197,253,0.2) inset',
          }}
        >
          {/* Logo */}
          <div className="flex items-center gap-2.5 shrink-0">
            <img src={logoTNUT} alt="TNUT" className="h-20 w-20 object-contain drop-shadow" />
          </div>

          {/* Divider */}
          <div className="h-5 w-px bg-blue-200/60 shrink-0" />
          ...
          <button
            ...
            className="ml-auto flex items-center gap-2 text-white text-sm font-semibold px-5 py-2 rounded-xl transition-all duration-200 hover:scale-[1.02] hover:brightness-110"
            style={{
              background: 'linear-gradient(135deg, #1e3a8a 0%, #2563eb 100%)',
              boxShadow: '0 4px 14px rgba(37,99,235,0.35), inset 0 1px 0 rgba(255,255,255,0.18)',
            }}
          >
            Đăng nhập
      ...
      <main className="relative z-10 flex-1">
        <section className="w-full px-10 pt-12 pb-12">
          <div className="grid grid-cols-[1fr_2fr_1fr] gap-10 items-start">
            ...
            <div className="flex flex-col justify-center pt-8 pb-4">
              ...
              <p className="mt-3 text-blue-800 font-semibold text-base">
              ...
              <div className="flex items-center gap-3 mt-6">
                <button
                  onClick={() => navigate('/login')}
                  className="flex items-center gap-2 bg-blue-900 hover:bg-blue-800 text-white font-semibold text-sm px-6 py-3 rounded-xl shadow-lg transition-colors"
                >
                  <BookOpen className="w-4 h-4" />
                  Bắt đầu ngay
                  <ArrowRight className="w-4 h-4" />
                </button>
                <button className="flex items-center gap-2 border border-blue-300 text-blue-700 hover:bg-blue-50 font-semibold text-sm px-5 py-3 rounded-xl transition-colors">
              ...
              {/* Stats */}
              <div className="grid grid-cols-4 gap-4 mt-10 bg-white/70 backdrop-blur rounded-2xl p-4 shadow-sm border border-blue-100">
                {STATS.map(s => (
                  <div key={s.label} className="flex flex-col items-center text-center gap-1">
                    <s.icon className="w-5 h-5 text-blue-600" />
                    <span className="text-lg font-black text-blue-900">{s.value}</span>
            ...
            {/* ── Center: Orbit ── */}
            <div className="relative flex items-center justify-center" style={{ height: 600, overflow: 'hidden' }}>
              ...
              {/* Track dots — 12 at 30° intervals, larger on icon positions */}
              ...
              {/* Spinning layer — all icons orbit together */}
                ...
                {FEATURE_ICONS.map(({ icon: Icon, label, sub }, i) => {
                  ...
                  return (
                    <div key={label} className="absolute" style={{
                      ...
                      {/* Icon centered at orbit tip */}
                      <div style={{
                        position: 'absolute',
                        left: ORBIT_R - ICON_BOX / 2,
                        top: -ICON_BOX / 2,
                      }}>
                        {/* Cancel arm angle */}
                        <div style={{ transform: `rotate(-${startDeg}deg)` }}>
                          {/* Cancel orbit rotation → stays upright */}
            ...
            <div className="flex flex-col gap-4">
                  ...
                  {NOTIFICATIONS.map(n => (
                    <div key={n.title} className="flex items-start gap-2.5 border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-700 font-medium leading-snug line-clamp-2 mt-0.5">
                          {n.title}
                        </p>
              ...
              {/* Quote Card */}
              <div className="bg-gradient-to-br from-blue-700 to-blue-900 rounded-2xl p-5 shadow-lg text-white">
        ...
        {/* ═══════════════════════ FEATURE CARDS ═══════════════════════ */}
        {/* ═══════════════════════ FEATURE STRIP ═══════════════════════ */}
        <div className="relative z-10 mx-10 mb-6" style={{
        ...
        <footer className="relative z-10  text-center">
          <p className="text-[10px] text-slate-400/70 tracking-widest">
