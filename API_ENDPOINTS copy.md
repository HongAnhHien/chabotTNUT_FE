...
| `quality.helpful_rate`, `quality.csat` | TAI-TNUT `GET /api/analytics/summary?subject_id=` |
...
|--------|------|--------|--------|
| POST | `/api/feedback` | FormData: `message_id`, `user_id`, `value` (`like`/`dislike`) | Ghi nhận like/dislike per message |
